...
\subsection{Just two Bernoulli random variables}
...
There's a \emph{natural} way to squeeze the tuple into one single parameter via log-odds ratio,
\begin{equation*}
	\delta:= \log \frac{\theta_a}{1-\theta_b} \Big/ \frac{\theta_b}{1-\theta_b} \in \Rf.
\end{equation*}
Note that this does not change two parameters are still needed 
but we just temporarily swept one of them into the bin for now.
Then we call the set of distributions on $(X,Y)$ parameterized by $\Pc_{\delta}$.
...
% FIXME: this subsection partially overlaps with "Setup and notation" below.
% Consider merging once the notation and problem statement are stabilized.
\subsection*{Again Bernoulli stream revisited}
...
FNCH gives
% TODO: write the explicit conditional distribution or remove this display.
\begin{align*}
